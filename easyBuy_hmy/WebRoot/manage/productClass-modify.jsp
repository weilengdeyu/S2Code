...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>修改分类</h2>
		<div class="manage">
			<form action="<%=path %>/servlet/doProductClassServlet?oper=modifytrue&id=${productCategory.id}" method="post">
				<table class="form">
					<tr>
						...
						<td>
							<select name="parentId">
								...
								<c:choose>
								<c:when test="${item.id eq productCategory.parentId}">
								<option value="${item.id }" selected="selected">${PCategory.name}</option>
								</c:when>
								<c:otherwise>
					...
					<tr>
						...
						<td><input type="text" class="text" name="className" value="${productCategory.name }" /></td>
					</tr>
					<tr>
						<td></td>
