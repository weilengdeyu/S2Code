...
<div id="main" class="wrap">
		...
		<div class="manage">
			<table class="list">
				...
				<tr>
					...
					<td class="w1 c">${item.name }</td>
					<c:choose>
					
						<c:when test="${item.sex eq 'T'}">
						<td>男</td>
						</c:when>
					...
					<td class="w1 c"><a href="<%=path %>/servlet/UserServlet?oper=modify&uid=${item.id }">修改</a> <a class="manageDel" href="<%=path %>/servlet/UserServlet?oper=del&uid=${item.id }">删除</a></td>
				</tr>
				</c:forEach>
	...
	<div class="clear"></div>
     <div class="pager">
				<ul class="clearfix">
					<li><a href="<%=path %>/servlet/UserServlet?oper=list&pageIndex=1">首页</a></li>
					<li><a href="<%=path %>/servlet/UserServlet?oper=list&pageIndex=${page.pageIndex-1}">上一页</a></li>
					<li>[${page.pageIndex }/${page.pageCount }]</li>
                    <li><a href="<%=path %>/servlet/UserServlet?oper=list&pageIndex=${page.pageIndex+1}">下一页</a></li>
					<li><a href="<%=path %>/servlet/UserServlet?oper=list&pageIndex=${page.pageCount}">尾页</a></li>
				</ul>
			</div>
</div>
<div id="footer">
