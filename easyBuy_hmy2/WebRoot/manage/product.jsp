...
<div id="main" class="wrap">
		...
		<div class="manage">
			<table class="list">
				...
				<tr>
					...
					<td class="thumb"><img src="<%=path %>/images/product/${item.fileName}" width="110" height="106"/><a href="<%=path %>/servlet/doProductServlet?oper=prodetital&proid=${item.id }&pid=${item.categoryId}" target="_self">${item.name }</a></td>
					<td class="w1 c"><a href="<%=path %>/servlet/doProductServlet?oper=houTaiModify&id=${item.id }">修改</a> <a class="manageDel" href="<%=path %>/servlet/doProductServlet?oper=houTaidel&id=${item.id }">删除</a></td>
				</tr>
				</c:forEach>
    ...
    <div class="pager">
				<ul class="clearfix">
					...
					<li><a href="${pageContext.request.contextPath}/servlet/doProductServlet?oper=proManage&pageIndex=1&id=<c:if test="${ empty id}">1</c:if><c:if test="${not empty id}">${id }</c:if>">首页</a></li>	
					<!-- 上一页-->	
					  <!-- 显示分页码-->
					  ...
					  <li>
					   <c:forEach  begin="${pages.listbegin}" end="${pages.listened}" var="i" varStatus="status">
                           ...
                           <c:otherwise>
                               <span><a href="${pageContext.request.contextPath}/servlet/doProductServlet?oper=proManage&pageIndex=${i}">${i}</a></span>
                           </c:otherwise>
                        </c:choose>
					  </c:forEach>
					  </li>
					<!-- 尾页 -->			
					<li><a href="${pageContext.request.contextPath}/servlet/doProductServlet?oper=proManage&pageIndex=${pages.totalPages}">尾页 </a></li>
					</c:if>		
