...
$(function(){
    ...
    $("#changeCode").click(function(){
        $("#safeCode").attr("src","/easyBuy_hmy/servlet/getCodeServlet?id="+Math.random());
    });
    //注册页面验证
...
//            location.href = "Cart?action=addAddress&address="+$("#addAddr").val();
        ...
        if(opr=="del"){
            ...
            if(number<=0){
                if(confirm("确定要删除吗？")) {
                	...
                	var url='/easyBuy_hmy/servlet/doCartShoppingServlet?oper=del&itemid='+id
                	del(url,$price);
                	...
                	//向后台发送一个ajax请求
              	  $.ajax({
                    	url:'/easyBuy_hmy/servlet/doCartShoppingServlet?oper=ProductNumber&do=jianNumber',
                    	type:"post",
                    	data:"id="+id,
                    	success:function(sucessdata1){
                    		$number.val(sucessdata1);
                    		$("#shopping").find("#total").text("总计：￥"+totalPrice()); 
                    	}	
                    });
                }
            }
        }else if(opr=="add"){//发送ajax请求  让数量加1
        	var id=$(this).parent().parent().attr("id");
            number++;
            //向后台发送一个请求
            $.ajax({
            	url:'/easyBuy_hmy/servlet/doCartShoppingServlet?oper=ProductNumber&do=addNumber',
            	type:"post",
            	data:"id="+id,
            	success:function(sucessdata){
            		$number.val(sucessdata);
            		$("#shopping").find("#total").text("总计：￥"+totalPrice());
    ...
    $("#logout").click(function(){
        if(confirm("购物车中尚有未结算的商品，是否注销？")) {
            location.href="/easyBuy_hmy/servlet/doLoginOut";
        }else{
            location.href="/easyBuy_hmy/servlet/doIndexServlet";
            return false;
        }
    });
    //轮换广告
    ...
    //(自行添加)用Ajax请求servlet取数据
    function showItems(table){
    	$.ajax({
	       url:'/easyBuy_hmy/servlet/doCartShoppingServlet?oper=allItems',
 	       type:'post',
 	       //回调函数 data代表server回送的数据
 	       success:function(data){
 	    	  var html=[
 	    	 ...
 	    	 $.each(eval("("+data+")"),function(i,item){
 	    		 html[i+1]= "<tr><td><img src='/easyBuy_hmy/images/product/"+item.product.fileName+"' width='50' height='40'/></td><td>"+item.product.name+"</td><td>"+item.product.price+"</td></tr>";           	 
              });
    ...
    //(自行添加)使用ajax去请求对应的servlet以获取数据
    function getchildByparent(num){
    	    $.ajax({
    	       url:'/easyBuy_hmy/servlet/doProductCategoryServlet?oper=select',
    	       type:'post',
    	       data:'num='+num,
    	       //回调函数 data代表server回送的数据
    	       success:function(data){
    ...
    $(".navbar li").mouseover(function(){
        ...
        //location.href="/easyBuy/servlet/doIndexServlet?oper=loadchildByParent&id="+$(this).attr("id");
        //alert($(this).attr("id"));
        //getchildByparent($(this).attr("id"));
     });
})